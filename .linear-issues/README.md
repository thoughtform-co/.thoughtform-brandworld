...
This folder contains issue documentation for the Thoughtform Brand World repository. It follows a consistent format used across all Thoughtform repositories (Atlas, Ledger, Astrolabe, etc.).
...
├── PREFIX-001-short-description.md    # Individual issue files
├── PREFIX-002-components/              # Folder for issue context
...
1. Copy `ISSUE_TEMPLATE.md` to a new file: `PREFIX-XXX-short-description.md`
2. Fill in the Problem section with clear description and user impact
3. Add to `OPEN_ISSUES.md` index
4. If investigation is needed, create a `PREFIX-XXX-components/` folder
...
✓ ATL-007: React Error #310 Infinite Loop on Modal Open
✗ Fix the bug
✗ Card issue
```
...
Framework upgrades cascade unpredictably. Spacing systems built on 
relative units (rem/em) create visual drift when fonts differ between 
environments. **The lesson:** for pixel-perfect design systems, use 
hard pixel values in critical spacing areas.
```
...
1. Reference the issue file: "See ATL-010 in .linear-issues/"
...
4. Reference issues by full ID (e.g., ATL-010, not just 010)
