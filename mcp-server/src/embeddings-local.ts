 ...
 */
export function initializeEmbeddings(config: { voyageKey: string }): void {
 ...
 */
function loadEmbeddingsIndex(): void {
 ...
 */
function saveEmbeddingsIndex(): void {
 ...
 */
export async function storeEmbedding(record: Omit<EmbeddingRecord, "id" | "embedding" | "created_at"> & { embedding?: number[] }): Promise<string> {
  if (!embeddingsIndex) {
    embeddingsIndex = {
      version: "1.0.0",
      model: "voyage-3-lite",
...
export async function searchEmbeddings(
  ...
  if (options.platforms && options.platforms.length > 0) {
    records = records.filter(r => 
      r.platforms?.some(p => options.platforms!.includes(p)) || 
...
export async function calculateDrift(
  ...
  const fingerprint = embeddingsIndex.records.find(
    r => r.category === "fingerprint" && r.source_id === platform
  );
 ...
 */
export async function activateAnchors(request: string): Promise<AnchorActivation> {
  if (!embeddingsIndex) {
    return {};
  }
 ...
 */
export async function detectPlatform(request: string): Promise<{
  platform: string;
  score: number;
  runner_up?: { platform: string; score: number };
}> {
  if (!embeddingsIndex) {
 ...
 */
export function getValidationStatus(driftScore: number): "approved" | "expansion" | "edge_case" | "violation" {
 ...
 * Get embedding stats
 */
export function getStats(): { configured: boolean; count: number; categories: Record<string, number> } {
  if (!embeddingsIndex) {
...
interface CaptainsLogIndex {
  version: string;
  model: string;
  count: number;
  updated_at: string;
  records: CaptainsLogEmbeddingRecord[];
}

let captainsLogIndex: CaptainsLogIndex | null = null;

/**
 * Load Captain's Log embeddings index from disk
 */
function loadCaptainsLogIndex(): void {
 ...
 */
function saveCaptainsLogIndex(): void {
  if (!captainsLogIndex) return;
...
export async function storeCaptainsLogEntry(entry: {
  ...
  const record: CaptainsLogEmbeddingRecord = {
    id: entry.id,
 ...
 */
export async function searchCaptainsLog(
  query: string,
  options: {
    platform?: string;
    context?: string;
