...
let embeddings: ComponentEmbedding[] = [];
let designSpace: { axes: DesignSpaceAxis[]; platforms: PlatformDefinition[] } | null = null;
let componentGraph: ComponentGraph | null = null;
let voyageApiKey: string | null = null;

export function initializeNavigator(config: { voyageKey?: string }): void {
...
function positionDistance(a: SemanticPosition, b: SemanticPosition): number {
  return Math.sqrt(
    Math.pow(a.terminal_organic - b.terminal_organic, 2) +
...
function interpolatePosition(a: SemanticPosition, b: SemanticPosition, ratio: number): SemanticPosition {
  return {
...
export async function matchReference(
  ...
  const implementationPath: string[] = [];
  if (matches.length > 0) {
...
export function searchByPosition(
  position: Partial<SemanticPosition>,
  options: { limit?: number } = {}
): MatchResult[] {
  const limit = options.limit || 5;
  
  // Create full position with defaults at 0
  const targetPosition: SemanticPosition = {
 ...
 */
export async function projectReference(
  description: string,
  ...
  for (const platformId of platforms) {
      ...
      ? platformComponents
          .map(comp => ({
            ...
            similarity: cosineSimilarity(
              baseMatch.matches[0].component.embedding,
              comp.embedding
            ),
          }))
          .sort((a, b) => b.similarity - a.similarity)
    ...
    const platformTokens = new Set<string>(platform.primaryTokens);
    const platformPatterns = new Set<string>();
    
    for (const sc of similarPlatformComps) {
      for (const t of sc.component.usesTokens) platformTokens.add(t);
      for (const p of sc.component.usesPatterns) platformPatterns.add(p);
    }
    ...
    switch (platformId) {
      case "atlas":
        platformDescription = `As Atlas: Organic, specimen-like treatment. ${
          baseMatch.suggestedPatterns.includes("cornerBrackets") ? "Corner brackets framing the element. " : ""
        }Dawn glow accents on void background. Breathing animation if interactive.`;
        break;
      case "ledger":
        platformDescription = `As Ledger: Terminal aesthetic with scanline overlay. Verde accent on key data points. Monospace typography, horizontal flow.${
          baseMatch.suggestedPatterns.includes("particleSystem") ? " Horizontal particle drift in background." : ""
        }`;
        break;
      case "astrolabe":
 ...
 */
export function interpolateDesigns(
  componentA: string,
  componentB: string,
  steps: number = 5
): InterpolationPoint[] {
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    if (position.terminal_organic < -0.3) description += ". Terminal aesthetic.";
    else if (position.terminal_organic > 0.3) description += ". Organic aesthetic.";
    
    if (position.cool_warm < -0.3) description += " Cool tones.";
    else if (position.cool_warm > 0.3) description += " Warm tones.";
...
export function exploreSpace(
  parameters: Partial<SemanticPosition>,
  options: { limit?: number } = {}
): ExploreResult {
  if (!designSpace) {
    throw new Error("Design space not loaded");
  ...
  for (const platform of designSpace.platforms) {
    const dist = positionDistance(position, platform.position as SemanticPosition);
  ...
  const suggestedTokens: string[] = [...nearestPlatform.primaryTokens];
  
  for (const axis of designSpace.axes) {
    const axisValue = position[axis.id as keyof SemanticPosition];
    if (axisValue < -0.3) {
...
export function getComponentById(id: string): ComponentEmbedding | undefined {
  return embeddings.find(c => c.id === id || c.name === id);
}

export function getAllComponents(): ComponentEmbedding[] {
  return embeddings;
}

export function getDesignSpace(): typeof designSpace {
  return designSpace;
}

export function getComponentGraph(): typeof componentGraph {
