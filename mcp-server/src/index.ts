...
import {
  ...
  searchEmbeddings,
  calculateDrift,
  activateAnchors as activateAnchorsEmbedding,
  detectPlatform as detectPlatformEmbedding,
  getStats as getEmbeddingStats,
  // Captain's Log embedding functions
  storeCaptainsLogEntry,
  searchCaptainsLog,
  getRecentCaptainsLogs,
  getCaptainsLogStats,
...
import {
  ...
  getDesignSpace,
  getComponentGraph,
  getStats as getNavigatorStats,
} from "./design-navigator.js";
...
function getConfiguredRepos(): Record<string, string> {
  const envRepos = process.env.THOUGHTFORM_REPOS;
  if (!envRepos) return DEFAULT_REPOS;
...
function isPathAllowed(targetPath: string): { allowed: boolean; repo?: string } {
...
function resolveRepoPath(repoOrPath: string, relativePath?: string): string | null {
...
function getValidationStatusLocal(driftScore: number): string {
...
function searchLocalContent(query: string, category?: string, limit: number = 5): unknown[] {
    ...
    for (const platform of platforms) {
      ...
      if (text.includes(queryLower)) {
        results.push({
          ...
          content: (fp.identity as { extended?: string })?.extended || "",
  ...
  if (!category || category === "anchor") {
    const anchors = readJsonFile("tokens/anchors/definitions.json") as { anchors?: Record<string, unknown> } | null;
    if (anchors?.anchors) {
      for (const [name, anchor] of Object.entries(anchors.anchors)) {
        ...
        if (text.includes(queryLower)) {
          results.push({
            ...
            source_id: name,
            title: `${name} anchor`,
            content: (anchor as { meaning?: string }).meaning || "",
            similarity: 0.7,
          });
        }
      }
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "search_references") {
    const { anchors, dialect, mode, tags } = args as {
      anchors?: string[];
      dialect?: string;
      mode?: string;
      tags?: string[];
    ...
    const keywordIndex = readJsonFile("references/index/keywords.json") as Record<string, any>;
    if (!keywordIndex) {
    ...
    if (anchors && anchors.length > 0) {
      results = results.filter(([_, entry]: [string, any]) => {
        const entryAnchors = entry.anchors?.highest || [];
        return anchors.some((a: string) => entryAnchors.includes(a));
      });
    }
    ...
    // Filter by mode
    if (mode) {
      results = results.filter(([_, entry]: [string, any]) => entry.referenceMode === mode);
    }
          ...
          text: JSON.stringify(
            {
              ...
              results: results.map(([id, entry]: [string, any]) => ({
                id,
                title: entry.title,
  ...
  if (name === "suggest_translation") {
    const { referenceId } = args as { referenceId: string };

    const keywordIndex = readJsonFile("references/index/keywords.json") as Record<string, any>;
    const translationTable = readJsonFile("semantic/translations/translation-table.json") as Record<string, any>;
    const assetsRegistry = readJsonFile("registry/assets.json") as Record<string, any>;
    ...
    const anchorTranslations: Record<string, string[]> = {};
    ...
    const matchingAssets = (assetsRegistry.assets || []).filter((asset: any) => {
      const assetTranslations = asset.translations || [];
      return suggestedTranslations.some((t: string) => assetTranslations.includes(t));
    });
          ...
          text: JSON.stringify(
            {
              ...
              anchorTranslations,
              matchingAssets: matchingAssets.map((a: any) => ({
                id: a.id,
                name: a.name,
                type: a.type,
  ...
  if (name === "get_translation_chain") {
    const { anchor } = args as { anchor: string };

    const translationTable = readJsonFile("semantic/translations/translation-table.json") as Record<string, any>;
    const anchorsData = readJsonFile("tokens/anchors/definitions.json") as Record<string, any>;
  ...
  if (name === "search_philosophy") {
    const { query, category, limit = 5 } = args as { query: string; category?: string; limit?: number };
  ...
  if (name === "check_platform_alignment") {
    const { platform, description } = args as { platform: string; description: string };
    
    if (!embeddingsConfigured) {
      // Fallback to fingerprint keyword matching
      const fingerprint = readJsonFile(`semantic/fingerprints/${platform}.json`) as Record<string, any>;
      if (!fingerprint) {
      ...
      const matchedKeywords = keywords.filter((k: string) => descLower.includes(k.toLowerCase()));
  ...
  if (name === "activate_anchors") {
    ...
    if (!embeddingsConfigured) {
      ...
      for (const [anchorName, anchor] of Object.entries(anchorsData.anchors)) {
        const anchorObj = anchor as { resonatesWith?: string[]; expressions?: string[] };
        const keywords = [
  ...
  if (name === "detect_platform") {
    ...
    if (!embeddingsConfigured) {
      ...
      const platforms = ["astrolabe", "atlas", "ledger-dark", "ledger-light", "marketing"];
      const scores: { platform: string; score: number }[] = [];
      const requestLower = request.toLowerCase();
      ...
      for (const platform of platforms) {
        ...
        const keywords = fingerprint.keywords || [];
        const matchedKeywords = keywords.filter((k: string) => requestLower.includes(k.toLowerCase()));
        scores.push({
          platform,
  ...
  if (name === "validate_design") {
    const { request, platform: specifiedPlatform } = args as { request: string; platform?: string };
    ...
    if (!platform) {
      if (embeddingsConfigured) {
        ...
        for (const p of platforms) {
          const fingerprint = readJsonFile(`semantic/fingerprints/${p}.json`) as Record<string, any>;
          if (!fingerprint) continue;
          const keywords = fingerprint.keywords || [];
          const matched = keywords.filter((k: string) => requestLower.includes(k.toLowerCase()));
    ...
    if (embeddingsConfigured) {
      ...
      for (const [anchorName, anchor] of Object.entries(anchorsData?.anchors || {})) {
        ...
        let matches = 0;
        for (const kw of keywords) {
          if (requestLower.includes(kw.toLowerCase())) matches++;
        }
    ...
    for (const anchor of topAnchors) {
      ...
      if (anchorTrans?.physicalPatterns) {
        for (const [, pattern] of Object.entries(anchorTrans.physicalPatterns).slice(0, 2)) {
          const p = pattern as { component?: string };
          if (p.component) suggestedComponents.push(p.component);
        }
      }
    }
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          ...
          interpretation: getDriftInterpretation(driftScore),
          activated_anchors: Object.fromEntries(
  ...
  if (name === "captains_log") {
    const { transcription, platform: platformOverride, context, source } = args as {
    ...
    const entry = createLogEntry(transcription, {
      platform: platformOverride && isValidPlatform(platformOverride) ? platformOverride as Platform : undefined,
      contextType: context && isValidContextType(context) ? context as ContextType : undefined,
      source: source && isValidSource(source) ? source as Source : 'mcp',
    });
  ...
  if (name === "search_captains_log") {
    const { query, platform, context, limit = 10 } = args as {
      query: string;
      platform?: string;
      context?: string;
      limit?: number;
  ...
  if (name === "get_log_timeline") {
    const { platform, limit = 20 } = args as {
      platform?: string;
      limit?: number;
    };
  ...
  if (name === "read_repo_file") {
    const { path: filePath, encoding = "utf-8" } = args as { path: string; encoding?: string };
  ...
  if (name === "search_repo_files") {
    const { pattern, repo } = args as { pattern: string; repo?: string };
    
    const searchRepos = repo ? { [repo]: THOUGHTFORM_REPOS[repo] } : THOUGHTFORM_REPOS;
    const results: Array<{ repo: string; path: string; relativePath: string }> = [];
    ...
    function searchDir(dir: string, repoName: string, repoBase: string): void {
      try {
  ...
  if (name === "get_repo_tree") {
    const { repo, maxDepth = 3 } = args as { repo: string; maxDepth?: number };
  ...
  if (name === "match_reference") {
    const { description, platform, limit = 5 } = args as {
      description: string;
      platform?: string;
      limit?: number;
    };
  ...
  if (name === "project_reference") {
    const { description, platforms } = args as {
      description: string;
      platforms?: string[];
    };
  ...
  if (name === "interpolate_designs") {
    const { componentA, componentB, steps = 5 } = args as {
      componentA: string;
      componentB: string;
      steps?: number;
    };
  ...
  if (name === "explore_space") {
    const { terminal_organic, minimal_dense, cool_warm, static_animated, limit = 5 } = args as {
