 ...
 */
export async function storeEmbedding(record: EmbeddingRecord): Promise<string> {
  if (!supabaseClient) {
 ...
 */
export async function storeFingerprint(fingerprint: PlatformFingerprint): Promise<string> {
  if (!supabaseClient) {
    throw new Error("Supabase not configured");
  }
...
export async function activateAnchors(request: string): Promise<AnchorActivation> {
  ...
  return data as AnchorActivation;
}
 ...
 */
export async function logDesignDecision(decision: DesignDecision): Promise<string> {
  if (!supabaseClient) {
    throw new Error("Supabase not configured");
  }
